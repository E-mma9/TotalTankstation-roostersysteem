    ...
    <div className="min-h-screen flex flex-col bg-slate-50">
      <header className="bg-white border-b border-slate-100 sticky top-0 z-20 shadow-sm">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 flex items-center justify-between h-[4.5rem]">
          ...
          {/* Rechts: notificaties + user + logout */}
          <div className="flex items-center gap-2">
            ...
            >
              ...
              {unreadCount > 0 && (
                <span className="absolute top-1.5 right-1.5 bg-red-500 text-white text-[10px] rounded-full min-w-[1.2rem] h-[1.2rem] flex items-center justify-center font-bold leading-none">
                  {unreadCount}
                </span>
              )}
            </NavLink>
        ...
        {menuOpen && (
          <nav className="lg:hidden border-t border-slate-100 px-4 py-3 space-y-1 bg-white">
            <p className="text-xs uppercase tracking-wide text-slate-400 px-3 py-1 font-semibold">Medewerker</p>
            {employeeLinks.map((l) => (
              <NavLink
                key={l.to}
                to={l.to}
            ...
            {isManager && (
              <>
                <p className="text-xs uppercase tracking-wide text-slate-400 px-3 py-1 mt-3 font-semibold">Manager</p>
                {managerLinks.map((l) => (
                  <NavLink
                    key={l.to}
                    to={l.to}
